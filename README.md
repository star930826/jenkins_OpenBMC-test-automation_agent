...
1. **SSH 金鑰**：檢查 `/home/jenkins/.ssh/id_ed25519` 是否存在
   - 不存在 → 自動產生一組新的（並印出公鑰，需手動貼到 Git remote / Jenkins Credentials）
   - 存在（通常是掛載自宿主機的資料夾）→ 沿用，不重新產生
...
- **`gen_misc.py`（openbmc-test-automation repo 內）** 的 `gm.which()` 若有路徑處理相關問題，修正應 commit 進該 repo，而非僅在 agent 端手動修補，否則每次 `cleanWs()` 後會失效。
